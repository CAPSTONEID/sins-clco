...
| `sins-palmierpro-cutedit.skill` | `/sins-palmierpro-cutedit` | PalmierPro MCP로 **현재 열려있는 프로젝트 타임라인을 직접 읽고 컷편집**. 무음 삭제는 **완료 기준 4개**(실무음 0.6초↑ 0곳 · 타임라인 갭 0 · 무발화 클립 0 · 10프레임 미만 파편 0)를 충족할 때까지 문턱을 1.0→0.6→0.35→0.25초로 낮춰가며 **최대 5회 루프**, 여백(`speechPaddingSeconds`)은 0.15초로 두어 모든 공백에 0.30초 호흡을 남긴다. 이어서 군소리('음'·'어'·'아') 제거 → 버벅임 반복 제거(마지막만 유지) → 반복 발화·미완결 테이크·헛시작 제거 → 무발화 클립·외국어 연속 발화 제거 → 컷 이음새 파편 정리. 무음은 `remove_silence` 네이티브 처리, 단어 룰은 인덱스로 모아 `remove_words` **한 번에** 반영(인덱스 밀림 방지). 한국어 오삭제 위험 때문에 `matches` 자동매칭 대신 인덱스 지목 방식, 삭제 목록 승인 후 실행. 편집 뒤 단어 수가 줄면 **A/B 대조로 ASR 이음새 오류와 실제 오디오 손실을 구분**한 다음에만 undo |
