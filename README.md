...
| `sins-video-consource.skill` | `/sins-video-consource` | 대본·오디오·영상·PPTX로 스크립트만 / 영상 자동 제작 / 스크립트+HTML을 만드는 워크플로우. **영상 기본 디자인은 `paper-briefing`**: 검은 배경, 미색 찢어진 종이, 붉은 밑줄과 절제된 순차 등장, **테이프 장식 없음**. 본문은 **마침표마다 줄바꿈**하고 긴 문장은 자연스럽게 한 번 더 나눈다. **텍스트만 있는 장면은 금지**이며 매 장면에 라인 아이콘·도해를 넣는다. 밑줄은 **내레이션이 그 어구를 말하는 순간** 그리고, 장면마다 등장 애니메이션을 다르게 쓴다. 핵심 문구·비교·과정·수치·근거의 5종 구성과 읽기 여백을 사용하고, 수치가 있을 때만 차트·막대를 만든다. 이 프로파일은 기존 골드·그리드·네온·무한 아이들 모션과 롱폼 고정 슬롯·최소 채움률 규칙에 우선한다. **디자인 변경·미리보기 요청은 제작 입력 질문 없이 진행**하고, 실제 영상은 필요한 입력과 숏폼 여부를 확인한다. `/hyperframes` 롱폼 2560×1440(자막 없음), 숏폼 1080×1920 2편(각 90초 이내), 숏폼 안전영역·자막·CTA 규칙은 유지한다. 롱폼 페이퍼 디자인은 `scripts/verify-paper-frame.js`, 기존 롱폼·숏폼은 `scripts/verify-frame.js`로 검수한다. 폰트는 국민대해공체 400 + Presentation 300. 옵션3 웹PT는 기존 1920×1080 마스터 프레임·키보드 전용 전환을 유지한다. [디자인 규칙](skill-list-docs/video-paper-briefing.md) |
